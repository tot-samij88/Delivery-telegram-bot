...
def get_markup(id):
    return utils.create_inlinemarkup(
        [[utils.create_inlinebutton(config.SENDER_NAME, url="t.me/{}".format(bot.get_chat(id).username))],
         [utils.create_inlinebutton(
             config.ADMIN_NAME, url="t.me/{}".format(config.ADMIN_LINK))],
...
def set_moderator(message):
    ...
    bot.send_message(config.ADMIN_ID, "Successfully added {} to moders! ✅".format(
        link(moder, user.first_name)), parse_mode=markdown)
...
def del_moderator(message):
    ...
    bot.send_message(config.ADMIN_ID, "Successfully delete {} from moders! ✅".format(
        link(moder, user.first_name)), parse_mode=markdown)
...
def prepare_transit(message):
    ...
    bussy_dict[message.chat.id] = (
        message.content_type, controll[message.content_type].file_id, message.caption, message.date)
...
def callback_inline(call):
    ...
    if to == "channel":
        MessageStack.insert_message(
            format, call.message.chat.id, target_id, file_id, caption, date)
        bot.edit_message_text("Post soon to appear on the channel! ✅",
                              call.message.chat.id, call.message.message_id)

    if to == "group":
        bot.edit_message_text(
            "Successfully! ✅", call.message.chat.id, call.message.message_id)
    ...
    for name, value in config.CHANNELS.items():
        for channelid in value:
            for format, senderid, channelid, photolink, caption, date in MessageStack.order_messages(channelid):
                ...
                method[format](channelid, photolink, caption=caption,
                               reply_markup=get_markup(senderid))
